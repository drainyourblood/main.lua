...
-- Авто Lighting (Pink Sky) с возможностью отключения
...
----------------------------------------------------
-- AUTO LIGHTING (PINK SKY) - с сохранением оригинала
----------------------------------------------------
...
-- Сохраняем оригинальную Sky, если она была
for _, v in pairs(Lighting:GetChildren()) do
    if v:IsA("Sky") then
        originalLighting.SkyProperties = {
            SkyboxBk = v.SkyboxBk,
            SkyboxDn = v.SkyboxDn,
            SkyboxFt = v.SkyboxFt,
            SkyboxLf = v.SkyboxLf,
            SkyboxRt = v.SkyboxRt,
            SkyboxUp = v.SkyboxUp,
            SunAngularSize = v.SunAngularSize,
            MoonAngularSize = v.MoonAngularSize,
...
local function applyPinkSky(enable)
    if enable then
        -- Удаляем старые Sky перед созданием новой
        for _, v in pairs(Lighting:GetChildren()) do
            if v:IsA("Sky") then
                v:Destroy()
            end
        ...
        -- Восстанавливаем оригинальное небо, если оно было
        for _, v in pairs(Lighting:GetChildren()) do
            if v:IsA("Sky") and v.Name == "PinkSky" then
                v:Destroy()
            end
        end
        
        if originalLighting.SkyProperties then
            local Sky = Instance.new("Sky")
            Sky.SkyboxBk = originalLighting.SkyProperties.SkyboxBk
            Sky.SkyboxDn = originalLighting.SkyProperties.SkyboxDn
            Sky.SkyboxFt = originalLighting.SkyProperties.SkyboxFt
            Sky.SkyboxLf = originalLighting.SkyProperties.SkyboxLf
            Sky.SkyboxRt = originalLighting.SkyProperties.SkyboxRt
            Sky.SkyboxUp = originalLighting.SkyProperties.SkyboxUp
            Sky.SunAngularSize = originalLighting.SkyProperties.SunAngularSize
            Sky.MoonAngularSize = originalLighting.SkyProperties.MoonAngularSize
            Sky.StarCount = originalLighting.SkyProperties.StarCount
            Sky.Parent = Lighting
        end
...
CreateToggle(contentFrames[2],"Pink Sky",40,function(state)
...
StarterGui:SetCore("SendNotification", {
    Title = "Загружено",
    Text = "AimBot, FOV, PinkSky & Teleport доступны в меню"
})
